...
import LoBTraits from 'army/legion_of_blood/traits'
import LoNTraits from 'army/legion_of_night/traits'
import LoSTraits from 'army/legion_of_sacrament/traits'
import SoulblightTraits from 'army/soulblight/traits'

const getGHoNTraits = () => GHoNTraits.map(x => appendTag(x, 'Grand Host of Nagash'))
const getLoBTraits = () => LoBTraits.map(x => appendTag(x, 'Legion of Blood'))
const getLoNTraits = () => LoNTraits.map(x => appendTag(x, 'Legion of Night'))
const getLoSTraits = () => LoSTraits.map(x => appendTag(x, 'Legion of Sacrament'))
const getSoulblightTraits = () => SoulblightTraits.map(x => appendTag(x, 'Soulblight'))
