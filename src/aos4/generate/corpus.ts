...
export interface CorpusReview {
  ...
  abilityKeywordOverrides?: CorpusAbilityKeywordOverride[]
}
...
const sourceRulesContextIds = (
  ...
  /**
   * A seasonal enhancement table is an additional option beside the same-named battletome table,
   * not a replacement for it. "Using the Scourge of Aqshy Rules" (eng_30-06): picking from a
   * seasonal artefacts or heroic-traits table means you pick "from that table instead of other
   * such tables available to your faction, not in addition to" — an exclusivity applied at the
   * pick, while every table stays legal in a General's Handbook 2026-27 army. Both same-named
   * groups therefore keep the seasonal context (the 2026-08-16 structural supersede dropped the
   * battletome table and made its enhancements unimportable, #1979); the builder disambiguates
   * them by grouping season-exclusive options under the season's own header.
   */
...
const officialEvidenceFor = (sourceRecordId: SourceRecordId, review: CorpusReview): SourceRecordId[] =>
  uniqueSorted([
    ...
    ...(review.warscrollKeywordOverrides ?? [])
      .filter(override => override.sourceRecordId === sourceRecordId)
      .flatMap(override => override.officialSourceRecordIds),
    ...(review.abilityKeywordOverrides ?? [])
      .filter(override => override.sourceRecordId === sourceRecordId)
      .flatMap(override => override.officialSourceRecordIds),
  ])
...
export const buildAos4Corpus = (
  ...
  review.normalizationDiagnosticPolicies
    ...
    .concat((review.abilityKeywordOverrides ?? []).flatMap(override => override.officialSourceRecordIds))
  ...
  const abilityKeywordOverrides = new Map(
    (review.abilityKeywordOverrides ?? []).map(override => [override.sourceRecordId, override])
  )
  ...
  const addAbility = (record: AbilityRecord, actor: Ability['actor']) => {
    ...
    entities.push({
      ...
      kind: 'ability',
      revision:
        timingOverride || textOverride || keywordOverride
          ? createHash('sha256')
              .update(
                [
                  entityRevision(record.meta),
