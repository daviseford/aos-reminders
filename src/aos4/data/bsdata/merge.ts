...
export interface BsDataRegimentOfRenownSourceInput {
  artifact: ArtifactManifestEntry
  /** Repository slug (e.g. BSData/age-of-sigmar-4th) used for commit-stable identity aliases. */
  repository: string
  facts: BsDataRegimentOfRenownFact[]
  /** Official page source records that establish this content (the printed regiment rules). */
  officialSourceRecordIds: SourceRecordId[]
}

const INCLUSION_NOTE = /^This Regiment of Renown can be included in the following factions: (.+)\.$/

/** `• 1 Mancrusher Gargant` → `{ name: 'Mancrusher Gargant', count: 1 }`. */
const officialMembers = (fact: GamesWorkshopRegimentOfRenownFact): Array<{ name: string; count: number }> =>
  fact.unitSummary.map(line => {
    const match = line.trim().match(/^•\s*(\d+)\s+(.+)$/)
    if (!match) {
      throw new Error(
        `Official regiment ${fact.name} lists an unreadable member line ${JSON.stringify(line)}`
      )
    }
    return { name: match[2].trim(), count: Number.parseInt(match[1], 10) }
  })

const memberKey = (members: Array<{ name: string; count: number }>): string =>
  members
    .map(member => `${member.count}×${canonical(member.name)}`)
    .sort()
    .join('|')

/**
 * Merge reviewed BSData Regiment of Renown facts into the current dataset (issue #1999).
 *
 * BSData supplies the regiment's rules text only. The one effective official regiment-of-renown
 * battle-profile row with the same name establishes everything else: the displayed name, the
 * points, the inclusion factions (its fixed inclusion sentence, each name resolved exactly), and
 * the members (its organisation lines, which must equal the BSData member links). Each member
 * must resolve to exactly one current-standard warscroll in the merged dataset, so this runs after
 * `mergeBsDataWarscrolls` and links the live post-replacement ids. Every join fails closed, and a
 * regiment a secondary page already classifies is never given a second source.
 *
 * The output has the shape the current-HTML merge gives a Wahapedia regiment datasheet, so the
 * reviewed `regimentsOfRenown` classification and the generation gates apply unchanged.
 */
export const mergeBsDataRegimentsOfRenown = (
  dataset: WahapediaDataset,
  reconciliation: WahapediaHtmlReconciliation,
  sources: BsDataRegimentOfRenownSourceInput[],
  officialFacts: GamesWorkshopBattleProfileFact[]
): BsDataMergeResult => {
  if (!sources.length) return { dataset, reconciliation }
  const officialRegiments = officialFacts.filter(
    (fact): fact is GamesWorkshopRegimentOfRenownFact => fact.kind === 'regiment-of-renown'
  )
  const factionsByName = new Map<string, string[]>()
  dataset.factions.forEach(faction => {
    const key = canonical(faction.name)
    factionsByName.set(key, [...(factionsByName.get(key) ?? []), faction.id])
  })
  const discrepancies = [...reconciliation.discrepancies]
  const newWarscrolls: WahapediaWarscrollRecord[] = []
  const newAbilities: WahapediaWarscrollAbilityRecord[] = []
  const newAvailability: WahapediaDataset['regimentOfRenownFactions'] = []
  ...
  sources.forEach(source => {
    ...
    ;[...source.facts]
      .sort((left, right) => left.section.localeCompare(right.section))
      .forEach(fact => {
        const anchors = officialRegiments.filter(
          candidate => candidate.context === 'standard' && canonical(candidate.name) === canonical(fact.name)
        )
        if (anchors.length !== 1) {
          throw new Error(
            `BSData regiment ${fact.name} matches ${anchors.length} effective official regiment-of-renown facts; ` +
              'a regiment ships only when exactly one official battle-profile row establishes it'
          )
        }
        const [official] = anchors
        const existing = dataset.warscrolls.filter(
          record => record.regimentOfRenown && canonical(record.name) === canonical(official.name)
        )
        if (existing.length) {
          throw new Error(
            `duplicate-regiment-of-renown-source: ${official.name} is already classified from ` +
              `${existing.map(record => record.meta.sourceRecordId).join(', ')}; a reviewer must pick one source`
          )
        }

        const inclusionNotes = official.notes.flatMap(note => {
          const match = note.trim().match(INCLUSION_NOTE)
          return match ? [match[1]] : []
        })
        if (inclusionNotes.length !== 1) {
          throw new Error(
            `Official regiment ${official.name} carries ${inclusionNotes.length} inclusion sentences; ` +
              'its availability cannot be established'
          )
        }
        const inclusionNames = inclusionNotes[0].split(/\s*,\s*/).map(name => name.trim())
        const inclusionFactionIds = inclusionNames.map(inclusionName => {
          const ids = factionsByName.get(canonical(inclusionName)) ?? []
          if (ids.length !== 1) {
            throw new Error(
              `Official regiment ${official.name} names an unknown inclusion faction: ${inclusionName}`
            )
          }
          return ids[0]
        })
        if (new Set(inclusionFactionIds).size !== inclusionFactionIds.length) {
          throw new Error(`Official regiment ${official.name} repeats an inclusion faction`)
        }

        const members = officialMembers(official)
        if (memberKey(members) !== memberKey(fact.members)) {
          throw new Error(
            `BSData regiment ${fact.name} members ${memberKey(fact.members)} disagree with the official ` +
              `organisation ${memberKey(members)}`
          )
        }
        const memberRecords = members.map(member => {
          const candidates = dataset.warscrolls.filter(
            record =>
              !record.regimentOfRenown &&
              canonical(record.name) === canonical(member.name) &&
              (record.meta.rulesContextKinds ?? []).includes('standard')
          )
          if (candidates.length !== 1) {
            throw new Error(
              `Official regiment ${official.name} member ${member.name} matches ${candidates.length} ` +
                'current-standard warscrolls; a member must resolve to exactly one current-standard warscroll'
            )
          }
          return candidates[0]
        })

        const warscrollId = bsDataGeneratedId(fact.sourceRecordId)
        const sourceUrl = `${source.artifact.finalUrl}#${fact.section}`
        if (fact.name !== official.name) {
          discrepancies.push({
            url: sourceUrl,
        ...
        newWarscrolls.push({
          ...
          regimentOfRenownMemberIds: Array.from(new Set(memberRecords.map(record => record.id))),
          name: official.name,
          // The carrier is the member's faction, like a Wahapedia regiment's collection page;
          // generation never lets a regiment's carrier offer it or feed its contexts.
          factionId: memberRecords[0].factionId,
          sourceId: '',
          legendHtml: '',
          regimentOptions: '',
          notesHtml: official.notes.join('; '),
          ...
          meta: meta(
            fact.sourceRecordId,
            fact.factChecksum,
            uniqueSortedIds([official.sourceRecordId, ...source.officialSourceRecordIds])
          ),
        })
        fact.abilities.forEach(ability => {
          const points = abilityPoints(ability)
          newAbilities.push({
            ...
            pointsType: points.pointsType,
            points: points.points,
            meta: meta(ability.sourceRecordId, ability.recordChecksum),
          })
        })
        inclusionFactionIds.forEach(factionId => {
          const value = { warscrollId, factionId, inclusion: official.factChecksum }
          newAvailability.push({
            ...
            meta: meta(
              sourceRecordId(
                'bsdata',
                `${source.artifact.checksum}:${fact.section}:availability:${factionId}`
              ),
              checksum(value),
              [official.sourceRecordId]
            ),
  ...
  return {
    dataset: {
      ...
      regimentOfRenownFactions: [...dataset.regimentOfRenownFactions, ...newAvailability],
    },
    reconciliation: {
      ...reconciliation,
      discrepancies: discrepancies.sort(
...
const uniqueSortedIds = (values: SourceRecordId[]): SourceRecordId[] =>
