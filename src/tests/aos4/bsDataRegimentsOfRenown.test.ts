...
interface FixtureOptions {
  ...
  /** How many times the Mancrusher member link is repeated under Krong's force. */
  krongMemberLinks?: number
}
...
<catalogue name="۞ Regiments of Renown" id="${catalogueId}" type="catalogue" xmlns="http://www.battlescribe.net/schema/catalogueSchema">
  <entryLinks>
    ${
      krongMemberLink
        ? Array.from({ length: krongMemberLinks }, () =>
            memberLink('Mancrusher Gargant', 'force-krong', krongMemberMax)
          ).join('\n')
        : ''
    }
...
describe('BSData Regiment of Renown extraction (issue #1999, Krong the Club)', () => {
  ...
  it('fails closed on every missing or ambiguous source shape', () => {
    ...
    // A repeated member link is ambiguous, never a summed count the merge would have to catch.
    expect(codes({ krongMemberLinks: 2 })).toEqual(['ambiguous-regiment-member'])
