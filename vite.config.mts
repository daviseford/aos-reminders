 ...
 * it. Precaching either would also download it before the worker could activate, which is the worst
 * case on the venue wifi that motivates offline support in the first place.
 *
...
/*
 * The source records — every reminder's provenance — are 53% of the corpus and are read by exactly
 ...
 */
const CATALOG_SOURCES_CHUNK_NAME = `${CATALOG_CHUNK_NAME}-sources`
...
const SOURCES_URL = ${JSON.stringify(sourcesUrl)}
// Both chunks live in one cache: the runtime route writes here and this module prunes it, so a
// second cache would mean two writers fighting over the same bookkeeping.
const CURRENT_URLS = [CATALOG_URL, SOURCES_URL].filter(Boolean)
...
const warmChunk = async url => {
  ...
  const response = await fetchChunk(url)
  if (!isCatalogModule(response)) {
    throw new Error('sw-extras: warm did not return the module; ' + url)
  }
    ...
    (async () => {
      ...
      try {
        ...
        for (const request of await cache.keys()) {
          const isCurrent = CURRENT_URLS.includes(new URL(request.url).pathname)
