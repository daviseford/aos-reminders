...
5. Diff the old and new runtime for ids present in both whose name changed (case, curly quotes, and punctuation aside). Each one is either a reviewed rename of the same rule or a positional slot now holding a different rule; for the latter, move the slot's alias to the identity of the rule it now prints (a removed entity of the same name and owner, or a fresh id when the rule is new) and let the retired rule's entry go. When the fresh id's natural hash is still held by the retired rule, mint it from the alias plus a revision seed. Check the diff against every runtime deployed since the last pairing, not only master: an id that was ownerless in master can still key a preference saved earlier.
6. Record the count and the zero-ambiguity result in the review paragraph the maintenance runbook keeps per radar review, so the next reviewer knows the identities were preserved deliberately.
...
The identity scheme is also positional for HTML abilities that have no export row to match by name (`…/ability:N`, faction-page `#faction-ability:<group>:ability:N`), so a rewritten page can leave an ability's canonical id bound to a different ability than before. That is a second identity bug, not cosmetic churn: reminder hides, notes, and order are stored under reminder occurrence ids (`reminder:<ability id>@<semantic timing>`), so a player's hide or note on the old rule silently lands on whatever the slot now prints, and `migrateAos4ReminderPreferences` will even re-key a stranded preference onto it when the timing differs. (An earlier version of this document said reminders were not stored by ability id; that was wrong.) The removed/added pairing below cannot see it, because the slot's alias already existed, so nothing was removed or added. Corpus 2026-09-25 (PR #2023) shipped six such rebinds before review caught them.
