...
4. **Let the strict gate enumerate the rest.** Copy the accepted manifest and review to a dated
   ...
   alias onto the existing entry instead (2026-08-25b: 786 rows, 0 ambiguous). That pairing cannot
   see a positional slot whose alias already existed, so also list ids present in both runtimes
   whose name changed: reminder preferences are keyed by ability id, so a slot now printing a
   different rule must move to that rule's identity (2026-09-25: six such slots, fixed in review).
   Two more
...
| 2026-08-29 | BSData lane, 1 event (`d7377e94` → `a3b951c6`) | No pinned file in range (no BSData pins remain since 2026-08-28b); four normalized changed lines, all revision bumps and roster-builder plumbing, no rules text; baseline advanced only |
| 2026-09-10 | Games Workshop lane, 5 events (4 new, 1 removed publication) | September 2026 Battletome: Sons of Behemat cycle: the faction battle-profile supplement accepted (points corrections applied to the nine carried-over units, July main-document rows superseded including, by name, the four re-published regiment-of-renown rows; four new units profile-only with reviewed deviations, issue #1999), the Armies of Renown pack re-pinned to its September 2026 publication with the King Brodd's Stomp rewrite recorded but not applied (it references the battletome Big/Little and Destructive Impulse keywords no accepted text source carries), the SoB Regiments of Renown and Stone Lobbas Spearhead packs classified as reference, and two provisional watches registered on the Wahapedia SoB pages; corpus 2026-09-10 |
| 2026-09-10 | BSData lane, 1 event (`a3b951c6` → `a01c6610`, 7 commits, 7 catalogues) | No pinned file in range (no BSData pins remain since 2026-08-28b) and no `communityWarscrollSources`, so no shipped record can move; the commits are BSData-side corrections to Wahapedia-sourced factions — Splinters of Sorcery UNLIMITED, Artycle 25 Point 6 passive, Zephyrspites MOVE, and Graceful Elusion Any Combat Phase were all already correct in the accepted runtime, the Thundershock fix repairs a BSData-only typo, and the Radiant Wings (Anvil of Apotheosis) and Ironsunz restriction changes target Legends content the corpus does not ship from BSData; per policy none are adoptable from the signal; baseline advanced only |
| 2026-09-12 | Games Workshop lane, 2 events (1 new, 1 removed publication) | GW re-published the September 2026 Sons of Behemat battle-profile supplement with corrected 90mm base sizes for Boss-stompers and Rock-hurlers (previously 80mm); re-pinned with publication identity continuity — the ledger delta was exactly those two facts plus the artifact re-key, and no count or disposition moved; corpus 2026-09-12 |
| 2026-09-15 | Wahapedia lane, 45 events (28 factions + 17 rules pages, all `removed-*`) | Phantom removals: Wahapedia moved its site navigation out of every page into the JS-loaded `/aos4/nav.html` fragment (announced by `#siteNav[data-nav-src]`), so the sentinel parsed zero navigation links and reported every pinned page as removed while `Last_update.csv` still matched; no shipped record moved. The observer and the source-inventory command now discover and fetch the fragment, and the navigation parse fails closed (operational `source-contract-changed`) when it finds zero faction or rules pages; lane cleared by a manual `source: all` run |
| 2026-09-22 | BSData lane, `gargants` branch pin (`2b7df92f`, issue #1999) | Sons of Behemat September 2026 battletome intake: four new units, six coherent legacy datasheet rewrites, the current faction package (4 battle formations, 6 heroic traits, 6 artefacts of power, Prayers of the World Titan), and the regular army-wide battle traits shipped provisionally from the pinned branch commit; Realm-shaking Rampage, Lore of Behemat, King Brodd's Stomp's own battle traits, Krong the Club, and the Stone Lobbas Spearhead stayed deferred (no canonical type record, no BSData Regiment-of-Renown intake path for Krong, whose rules text BSData does carry, or no secondary at all for Stone Lobbas). Watches corrected: Krong moved to an inclusion-faction page (`blades-of-khorne`), a Stone Lobbas needle added, and four raw `gargants`/`main` BSData file watches added to catch the eventual merge. Independently found and fixed while observing: Wahapedia dropped the `.datasheetsCollated` link block from every faction root page, silently breaking `discoverWahapediaWarscrollCollection`'s ability to find any of the 27 accepted warscroll collections; it now derives the conventional `<faction root>warscrolls.html` path instead, restoring source-inventory completeness; corpus 2026-09-22 |
| 2026-09-22 | BSData lane, 1 event (`a01c6610` → `d989a15e`, 4 commits, 133 catalogues) | Whole-repo editor reformat (CRLF→LF, attribute/section reorder, mixed-content reorder); after canonical normalization 129 paths unchanged, 3 Wahapedia-sourced catalogues carry corrections the runtime already ships (Cities allied Auric Hearthguard/Vulkyn Flameseekers, Lord of Change summon range 12", Auric Runesmiter Emergence plural), 1 Regiments of Renown roster condition; the three gargants-pinned files are formatting-only on main and the 2b7df92f pin already sits on the reformat (merge base f26d7215); baseline advanced only |
| 2026-09-23 | Provisional watch, all four `bsdata-main-*` watches (issue #1999); range `d989a15e` → `8836d9f9` (1 squash commit, BSData PR #1359, 33 files) | BSData merged and deleted `gargants`. Accepted pins held: `Sons of Behemat - Library.cat` byte-identical at pin and `main`, `Sons of Behemat.cat` and `Lores.cat` differ only outside the consumed scopes. But the squash also carried BSData's implementation of the Games Workshop September 2026 update published the same day (Rules Updates, core Battle Profiles, six Scourge of Aqshy packs, a Cities of Sigmar supplement), and the runtime still ships the superseded wording in at least ten Wahapedia-sourced factions, including the seasonal Sons of Behemat content. Four Sons of Behemat regiments also ship text older than the already-pinned official Regiments of Renown pack. **Baseline held** at `d989a15e` until the official September 2026 intake reconciles those records; the four fired watches retired and the two Wahapedia watch reasons corrected |
| 2026-09-23 | Games Workshop lane, 20 events (10 new, 10 removed publications; lane fingerprint `71ba4e2b`), re-frozen unchanged at 21:58Z | The 2026-09-23 Games Workshop update: September 2026 Rules Updates, core Battle Profiles, six Scourge of Aqshy packs, the re-published Cities of Sigmar supplement, and the Bubonic Cell Spearhead replaced their pinned predecessors with publication identity continuity (107 citations re-keyed). Wahapedia had republished none of the affected pages, so 17 ability-text and 4 timing overrides cite the September page records; 11 further errata the machine review cannot verify (deletion-only, composed, or outside the minimized excerpt) and every change outside the override vocabulary (renames, keyword additions, characteristic and weapon-set changes, a command-point cost removal) stay recorded on #1999. The core Battle Profiles re-flowed its pages, so the extractor now reads each page's section from its printed title; it re-publishes every supplement row the runtime used, so it became the single battle-profile source and Krong the Club keeps one official anchor. The BSData baseline stays at `d989a15e` because material deltas remain unresolved; corpus 2026-09-23 |
| 2026-09-24 | Krong the Club intake (issue #1999), no radar event; BSData `Regiments of Renown.cat` pinned at `main` `8836d9f9` (blob `c4379f8b` unchanged at the later `main` head `d1f97bc9`, re-checked 2026-09-24) | First Regiment of Renown shipped from BSData, rebuilt on corpus 2026-09-23 after the official September intake: a bounded extract takes only Krong's two abilities from the catalogue (both match the official Regiments of Renown - Sons of Behemat pack, page 5, word for word), and the one effective official row, the September 2026 core Battle Profiles page 59, supplies its 140 points, 24 inclusion factions, and the single Mancrusher Gargant member; the BSData member link is a fail-closed cross-check and the game-system force-entry conditions are not read. The Wahapedia Krong watch became a duplicate-source sentinel. Certified against fresh same-day observations of all three publishers. `bsData.baselineSha` stays held at `d989a15e` (no approved disposition for the `d989a15e` → `d1f97bc9` range); corpus 2026-09-24 |
| 2026-09-24 | BSData lane, 1 event (`d989a15e` → `d1f97bc9`, 2 commits, 33 files; aggregate `87941e25`) | Owner-approved disposition of the held range. `8836d9f9` was reduced on 2026-09-23; the one new commit, `d1f97bc9`, changes a Pyregheists category link (`CHAMPION (1/8)` → `CHAMPION`) on a Wahapedia-sourced warscroll the runtime already ships with `CHAMPION`. All four pinned files are unchanged at the observed head. Baseline advanced only; this does not certify shipped rules correctness, and the unresolved official September 2026 discrepancies stay open on #1999 for an official-first candidate cycle |
| 2026-09-25 | Provisional watch, both Wahapedia watches (issue #1999); all 72 accepted Wahapedia pages had drifted in bytes with `Last_update.csv` and the exports unchanged, so the radar lane stayed quiet | Wahapedia republished the Sons of Behemat faction and warscroll collection pages with the September 2026 battletome (the faction-package watch reported `Destructive Impulse` absent only because the page prints it in capitals). Only those two pages re-pinned; the battletome now ships from them, the three BSData `gargants` catalogues, their community entries, eleven index-era `contextOverrides` that would have mis-applied to the battletome records at the same ids, and the Always On Guard override retired; eight new `contextOverrides` keep the Scourge of Ghyran formations and heroic traits historical and five new add-only `abilityKeywordOverrides` restore the RAMPAGE keyword official Armies of Renown pages 23-24 print on the King Brodd’s Stomp Destructive Impulse traits; Lore of Behemat, the six Realm-shaking Rampages, the King Brodd’s Stomp rewrite, the Stone Lobbas Spearhead, Matriarch’s Mob and Stomper Tribe (`secondary-provisional`), and Okar’s Torrbad ship; 88 re-sourced entities kept their canonical ids, 0 ambiguous, and after review the reused positional page slots were rebound so no shipped id names a different rule (certification `machine-r2`). Krong the Club, now on all 24 inclusion-faction pages, matches official page 5 except an extra Legion of the First Prince inclusion entry, so it stays on BSData and no inclusion page was re-pinned. The certification inventory also caught a same-day Games Workshop re-upload of the September core Battle Profiles (Khainite Shadowstalkers 100 to 130 points), re-pinned with publication identity continuity. Both watches retired; corpus 2026-09-25 |
